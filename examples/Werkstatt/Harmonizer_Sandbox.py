...
harm = Harmonizer(eq_Bass, transpo=[0.007,-0.007], feedback=0.05, winsize=0.0075, mul=0.6)
harm.ctrl()

pva = PVAnal(harm, size=1024)
pvt = PVTranspose(pva, transpo=1)
pvs = PVSynth(pvt).out()
dry = Delay(eq_Bass, delay=1024./s.getSamplingRate(), mul=.7).out()
dry.ctrl()
